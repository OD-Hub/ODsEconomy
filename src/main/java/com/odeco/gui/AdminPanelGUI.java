...
public class AdminPanelGUI implements InventoryHolder {
    ...
    private void populateTransactions(EconomyManager economy) {
        ...
        var transactions = economy.getAllTransactions();
        int totalPages = Math.max(1, (int) Math.ceil(transactions.size() / 36.0));
        int page = pageOffset + 1;

        inventory.setItem(4, new ItemBuilder(Material.PLAYER_HEAD)
                .name(ColorUtils.color("<blue>Recent Transactions</blue>"))
                .skull(Heads.TRANSACTIONS)
                .lore(ColorUtils.color("<gray>Page " + page + " of " + totalPages + " (" + transactions.size() + " total)</gray>"))
                .build());
        ...
        for (int i = start; i < Math.min(transactions.size(), start + 36); i++) {
            ...
            head = new ItemBuilder(head)
                    ...
                    .lore(
                            ColorUtils.color(color + typeName + "<reset>"),
        ...
        inventory.setItem(48, new ItemBuilder(Material.LAVA_BUCKET)
                .name(ColorUtils.color("<red>Clear History</red>"))
                .lore(ColorUtils.color("<gray>Delete all transaction records</gray>"))
                .build());
    }
    ...
    private void handleTransactionsClick(int slot, Player clicker) {
        ...
        if (slot == 48) {
            plugin.getEconomyManager().clearTransactionLog();
            clicker.sendMessage(ColorUtils.color("<green>Transaction history cleared.</green>"));
            pageOffset = 0;
            populate();
        }
    }
